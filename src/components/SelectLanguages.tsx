...
const SelectLanguages: React.FC<SelectLanguagesProps> = ({ onChange, value }) => {
  const [selected, setSelected] = useState(languages[0]);

  useEffect(() => {
    const selectedLanguage = languages.find((lang) => lang.id === value);
    if (selectedLanguage) setSelected(selectedLanguage);
  }, [value]);
      ...
      <div className="relative">
        <ListboxButton className="relative w-[150px] cursor-default rounded-md bg-dark py-1.5 pl-3 pr-10 text-left text-white shadow-sm ring-1 ring-inset ring-gray-300 focus:outline-none focus:ring-2 focus:ring-accent sm:text-sm sm:leading-6">
          <span className="flex items-center">
            {selected?.avatar && <img src={selected.avatar} alt="Selected language" className="h-5 w-5 flex-shrink-0 rounded-full" />}
        ...
        <ListboxOptions className="absolute z-10 mt-1 max-h-56 w-[150px] overflow-auto rounded-md bg-black py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
          {languages.map((language) => (
            ...
            >
              <div className="flex items-center">
                <img src={language.avatar} alt={language.name} className="h-5 w-5 flex-shrink-0 rounded-full" />
                <span className="ml-3 block truncate font-normal group-data-[selected]:font-semibold">{language.name}</span>
              </div>
